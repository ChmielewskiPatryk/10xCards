...
export default function FlashcardsPage() {
  ...
  const [toast, setToast] = React.useState<{ message: string; type: ToastType; open: boolean }>({
    message: "",
    type: "success",
    open: false,
  });
  ...
  const handleOrderChange = (value: Filters["order"]) => {
    setFilters((f) => ({ ...f, order: value }));
    setPage(1);
  };
  ...
  const handleSaveEdit = async (updatedFlashcard: FlashcardCandidate) => {
    ...
    try {
      const flashcard = data.find(
        (f) => f.front_content === editingFlashcard.front_content && f.back_content === editingFlashcard.back_content
      );
      if (!flashcard) {
      ...
      setToast({
        message: error instanceof Error ? error.message : "Błąd podczas aktualizacji fiszki",
        type: "error",
        open: true,
      });
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    if (deleteId) {
      try {
        ...
        setToast({
          message: error instanceof Error ? error.message : "Błąd podczas usuwania",
          type: "error",
          open: true,
        });
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      {editingFlashcard && (
        <FlashcardEditForm flashcard={editingFlashcard} onSave={handleSaveEdit} onCancel={handleCancelEdit} />
      )}
