...
export function FlashcardCard({ flashcard, onEdit, onDelete }: FlashcardCardProps) {
  return (
    <div className="border rounded-md p-4 flex flex-col gap-2" data-testid={`flashcard-${flashcard.id}`}>
      <div className="font-medium text-lg" data-testid={`flashcard-front-${flashcard.id}`}>{flashcard.front_content}</div>
      <div className="text-gray-600" data-testid={`flashcard-back-${flashcard.id}`}>{flashcard.back_content}</div>
