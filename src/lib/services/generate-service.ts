...
export class GenerateService {
  ...
  private async callAIService(sourceText: string, maxFlashcards: number): Promise<any> {
    ...
    if (this.useMockResponse || apiNotConfigured) {
      ...
      console.log(
        `Używanie odpowiedzi mockowej (Powód: ${reason}). Miało zostać wygenerowanych ${maxFlashcards} fiszek z ${sourceText.length} znaków tekstu`
      );
        ...
        messages: [
          {
            role: "system",
            content:
              "Jesteś polskim ekspertem specjalizującym się w tworzeniu fiszek edukacyjnych na podstawie podanego tekstu. Twórz zwięzłe pary pytanie-odpowiedź, które efektywnie sprawdzają zrozumienie kluczowych pojęć. Wszystkie fiszki MUSZĄ być w języku polskim. Nie używaj angielskiego ani żadnego innego języka w swoich odpowiedziach.",
          },
          {
            role: "user",
            content: prompt,
