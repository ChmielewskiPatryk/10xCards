...
export class FlashcardService {
  ...
  async approveFlashcards(
    ...
    const flashcardsToInsert = flashcards.map(flashcard => {
      ...
      const isEdited = directWasEdited || metadataWasEdited || 
        !!(flashcard.ai_metadata?.modified) || 
        !!(flashcard.ai_metadata?.edited_at);
    ...
    for (let i = 0; i < flashcardsToInsert.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        throw new Error(`Failed to save flashcards batch ${i / BATCH_SIZE + 1}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
