...
describe("GET /api/flashcards", () => {
  ...
  it("should validate query parameters correctly", () => {
    // Valid parameters
    expect(
      getFlashcardsQuerySchema.safeParse({
        page: 2,
        limit: 30,
        sort: "created_at",
        order: "asc",
  ...
  it("should return 400 for invalid query parameters", async () => {
    ...
    expect(loggerService.logError).toHaveBeenCalledWith(expect.any(String), "VALIDATION_FAILED", expect.any(String));
  });
  ...
  it("should return 500 when service throws an error", async () => {
    ...
    expect(loggerService.logError).toHaveBeenCalledWith(expect.any(String), "INTERNAL_ERROR", "Database error");
  });
});
...
describe("POST /api/flashcards", () => {
  ...
  it("should return 500 when service throws an error", async () => {
    ...
    const response = await POST(context as any);
    expect(response.status).toBe(500);
    expect(loggerService.logError).toHaveBeenCalledWith(expect.any(String), "INTERNAL_ERROR", "DBError");
  });
});
