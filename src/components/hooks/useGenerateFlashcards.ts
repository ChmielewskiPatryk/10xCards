...
import type { FlashcardCandidate, GenerateFlashcardsResponse, ApproveFlashcardsResponse } from "../../types";
import type { FlashcardCandidateViewModel, GenerationState, GenerateFlashcardsFormData } from "../generate/types";
...
export const useGenerateFlashcards = () => {
  ...
  const handleEditFlashcard = (index: number) => {
    setGenerationState((prev) => ({
      ...prev,
      flashcards: prev.flashcards.map((flashcard, i) => (i === index ? { ...flashcard, isEditing: true } : flashcard)),
    }));
  };
  ...
  const handleCancelEdit = (index: number) => {
    setGenerationState((prev) => ({
      ...prev,
      flashcards: prev.flashcards.map((flashcard, i) => (i === index ? { ...flashcard, isEditing: false } : flashcard)),
    }));
  };
